...
def make_map(data, filename):
	...
	# Make Feature groups for layers
	fg_open = folium.FeatureGroup(name="Open", show=True)
	fg_other = folium.FeatureGroup(name="Other", show=True)
	fg_closed = folium.FeatureGroup(name="Closed", show=True)

	m.add_child(fg_open)
	m.add_child(fg_other)
	m.add_child(fg_closed)

	# cluster_open = folium.MarkerCluster()
	# fg_open.add_child(cluster_open)
	# cluster_other = folium.MarkerCluster()
	# fg_open.add_child(cluster_open)	
	# cluster_closed= folium.MarkerCluster()
	# fg_open.add_child(cluster_open)
	...
	for park in data:
		...
		marker = folium.Marker(
			...
			popup = popup,
			name = status,
			icon = folium.Icon(icon = 'tree',prefix = 'fa',color = i_color))

		if status == "closed":
			marker.add_to(fg_closed)
		elif status == "open":
			marker.add_to(fg_open)
		else:
